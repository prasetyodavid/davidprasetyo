...
(function($, window, document, undefined) {
    $.fn.socialfeed = function(_options) {
        ...
        var defaults = {
            ...
            template: 'https://prasetyodavid.github.io/davidprasetyo/new_wall_template.html', // a path to the template file
        ...
        var options = $.extend(defaults, _options),
            container = $(this),
            template,
            social_networks = ['facebook', 'instagram', 'vk', 'google', 'blogspot', 'twitter', 'pinterest', 'rss', 'custom_facebook']
        ...
        var Feed = {
            ...
            facebook: {
                ...
                getData: function(account) {
                    ...
                    var fields = '?fields=posts.limit('+options.facebook.limit+'),id,name,link';
                       fields += (options.show_media === true)?',picture{url}':'';
                    var request_url, limit = '',
                    ...
                    switch (account[0]) {
                        case '@':
                            ...
                            Feed.facebook.utility.getUserId(username, function(userdata) {
                                if (userdata.id !== '') {
                                    request_url = Feed.facebook.graph + 'v2.4/' + userdata.id + '' + fields + limit + query_extention;
                                    proceed(request_url);
                                }
                            });
                            break;
                        case '!':
                            ...
                            break;
                        default:
                            request_url = Feed.facebook.graph + 'v2.4/' + account + fields + limit + query_extention;
                            proceed(request_url);
                            break;
                    }
                },
                ...
                utility: {
                    ...
                    getUsers: function(json) {
                        ...
                        json.data.forEach(function(user) {
                            var userid =1393380568;
                            var url = Feed.instagram.api + 'users/' + userid + '/media/recent/?' + authTokenParams + '&' + 'count=' + options.instagram.limit + '&callback=?';
                            Utility.request(url, Feed.instagram.utility.getImages);
                        });
                    },
                    unifyPostData: function(element) {
            ...
            blogspot: {
                ...
                utility: {
                    getPosts: function(json) {
                        $.each(json.feed.entry, function(idx, entry) {
                            var post = new SocialFeedPost('blogspot', Feed.blogspot.utility.unifyPostData(entry));
                            post.render();
                        });
                    },
                   ...
                   unifyPostData: function(element){
                        ...
                        var desc = jQuery(element.content['$t']).text();
                        var trimmedString = desc.substr(0, 130);
            ...
            custom_facebook : {
                ...
                getData: function(url) {
                    var limit = options.custom_facebook.limit,
                      yql = encodeURIComponent('select entry FROM feednormalizer where url=\'' + url + '\' AND output=\'atom_1.0\' | truncate(count=' + limit + ')' ),
                      request_url = Feed.custom_facebook.api + yql + '&format=json&callback=?';

                    Utility.request(request_url, Feed.custom_facebook.utility.getPosts, Feed.custom_facebook.datatype);
                },
                utility: {
                    ...
                    getPosts: function(json) {
                        if (json.query.count > 0 ){
                            $.each(json.query.results.feed, function(index, element) {
                                var post = new SocialFeedPost('custom_facebook', Feed.custom_facebook.utility.unifyPostData(index, element));
                                post.render();
                            });
                        }
                    },
