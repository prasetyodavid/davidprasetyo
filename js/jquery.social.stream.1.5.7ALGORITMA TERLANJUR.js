...
(function($){
	...
	var _process = function(e, data, opts) {
		...
		for (var i=opts.offset; i<feeds.entries.length; i++) {
			...
			if (opts.date && pubDate) rowArray[rowIndex]['html'] += '<div>'+ pubDate +'</div>'
			if (opts.content) {
...
//---------------------------------------------------------------------------------------------------------
	...
	function getFeed(type,id,path,o,obj,opt,f1,f2,intro,feed,fn){
	  ...
	  xxurl = 'https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&output=json_xml&num=10&callback=?&q='+ encodeURIComponent(url);
						...
						if(i < n){
							...
							$.each(o.out.split(','), function(i,v){
								...
								switch(v)
								{
									...
									case 'blogger_thumb':
    									src = item.content.indexOf("img") >= 0 ? $('img',item.content).attr('src') : '' ;
										...
										if (host == '2.bp.blogspot.com' || host == '3.bp.blogspot.com' || host == '4.bp.blogspot.com' || host == '1.bp.blogspot.com'){
											var img_src = 'http://'+host+'/'+urlChunks[urlChunks.length - 6]+'/'+urlChunks[urlChunks.length - 5]+'/'+urlChunks[urlChunks.length - 4]
														  +'/'+urlChunks[urlChunks.length - 3]+'/'+'s200'+'/'+urlChunks[urlChunks.length - 1];
											y = src ? srcx + ' <a href="'+q+'" class="thumb"><img height="auto" width="100%" src="'+img_src+'" style="border: 1px solid #ccc;"/></a>' : '' ;
										}else{
											y = src ? srcx + ' <a href="'+q+'" class="thumb"><img align="middle" height="auto" width="100%" src="'+src+'" alt="" style="border: 1px solid #ccc;"/></a>' : '' ;
										}
									zz += y;
									break;
									...
									case 'thumb_insta':
										var src = item.content.indexOf("img") >= 0 ? $('img',item.content).attr('src') : '' ;
										var urlChunks = src.split('/');
										var host = urlChunks[urlChunks.length - 6];
										if (host == 'scontent.cdninstagram.com'){
											var img_src = 'https://'+host+'/'+urlChunks[urlChunks.length - 5]+'/'+urlChunks[urlChunks.length - 4]+'/'+'s200x200'+'/'+urlChunks[urlChunks.length - 2]+'/'+urlChunks[urlChunks.length - 1];
											y = src ? '<a href="'+q+'" class="thumb"><img height="auto" width="100%" src="'+img_src+'" style="border: 1px solid #ccc;"/></a>' : '' ;
										}else{
									...
									case 'thumb_gplus':
										var src = item.content.indexOf("img") >= 0 ? $('img',item.content).attr('src') : '' ;
										var urlChunks = src.split('/');
										var urlProxy = src.split('=');
										var size = /[^=]*$/.exec(q)[0];

										var host = urlChunks[urlChunks.length - 3];
										var urlProxy = urlProxy[urlProxy.length - 2];

										if (host == 'lh6.googleusercontent.com' || host == 'lh4.googleusercontent.com' || host == 'lh7.googleusercontent.com' || host == 'lh5.googleusercontent.com' || host == 'lh3.googleusercontent.com'){
											var img_src = urlProxy+'='+'w210-h110';
											y = src ? '<a href="'+q+'" class="thumb"><img height="auto" width="100%" src="'+img_src+'" style="border: 1px solid #ccc;"/></a>' : '' ;
										}else{
											y = src ? '<a href="'+q+'" class="thumb"><img align="middle" height="auto" width="100%" src="'+src+'" alt="" style="border: 1px solid #ccc;"/></a>' : '' ;
										}
									...
									case 'thumb_enc':
										...
										yz = '<a href="'+q+'" class="thumb"><img height="auto" width="50%" src="'+global_media[xi]+':thumb" alt="" style="border: 1px solid #ccc;"/></a>' ;
										}
									zz += y+yz;
